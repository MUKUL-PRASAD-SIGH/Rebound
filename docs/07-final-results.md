...
- Dry-run/simulated execution by default, plus a guarded **MVP-mode** standard Payment Link executor backed by Razorpay Test Mode.
...
| --- | --- | --- |
| Backend regression suite | **37 passed** on 5 Sep 2026 | Core workflow, policy, evaluation, webhook-signature/idempotency, Payment Link reconciliation, read-only Razorpay reads, and LLM fallback are checked locally. |
...
| Razorpay execution | MVP-mode Payment Link implementation, pending-to-paid reconciliation, and mocked HTTP coverage | A live Test Mode creation/payment plus public webhook capture or authenticated status refresh remains the final integration proof. |

## Locked-scope assessment

The recovery domain, explicit simulations, fixed baselines, and policy-gated execution are complete for the MVP. The external Razorpay account demonstration and direct subscription/invoice/dashboard reads are intentionally still outside the evidence boundary. This is a credible, MVP-mode-first demo MVP—not a claim of production performance.
...
4. The public README was reshaped around the product, evidence, safe local run, and MVP mode (Razorpay Test Mode) validation rather than internal submission logistics.

## What we would do next

1. Run one controlled Payment Link success/failure and signed `payment_link` webhook in Razorpay Test Mode.
2. Exercise the existing read-only subscription/invoice reads against merchant-authorised Test Mode objects and normalise their event shapes into recovery cases.
