...
| Regression confidence | `cd src && python -m pytest tests -q` returned **32 passed** on 5 Sep 2026. | Met locally |
...
| Real Razorpay side effect | Executor has a guarded standard Payment Link test-mode path; HTTP behaviour is covered without live credentials. | Implementation ready; account proof pending |
| Full external outcome loop | One test Payment Link payment plus signed public webhook delivery. | Pending |
...
| Test-mode keys absent or non-test | Payment Link remains dry-run or is rejected before an external call |

## Evidence boundary

The next highest-value demo artifact is not another synthetic chart. It is a single controlled Test Mode Payment Link and signed webhook capture, labelled **test mode**, followed by the existing synthetic evaluation story. The operating procedure is in [`README.md`](../README.md#test-razorpay-safely-no-production-mode).
